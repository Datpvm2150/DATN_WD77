...
class BaiVietController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $param['user_id'] = auth('web')->id(); // Lấy ID của người dùng hiện tại
    ...
    {
        // Lấy bài viết theo ID cùng với thông tin người dùng và danh mục
        $baiViet = BaiViet::with(['user', 'danhMuc'])->findOrFail($id);

        // Tiêu đề cho trang xem chi tiết
        $title = "Chi tiết bài viết: " . $baiViet->tieu_de;

        // Trả về view với dữ liệu bài viết
        return view('admins.baiviets.show', compact('title', 'baiViet'));
    }
}
