...
class TaiKhoanController extends Controller
{
    ...
    public function index()
    {
        ...
        // Lấy thông trạng thái mặc định "chờ xác nhận" - sắp xếp mới nhất lên trên
        $donHangs = $user->hoaDons()->where('trang_thai', 1)->orderByDesc('created_at')->get();
        // Lấy thuộc tính
        $trang_thai_don_hang = HoaDon::TRANG_THAI;

        // Đếm số lượng theo từng trạng thái
        $counts = [
            1 => $user->hoaDons()->where('trang_thai', 1)->count(), // Chờ xác nhận
            2 => $user->hoaDons()->where('trang_thai', 2)->count(), // Đã xác nhận
            3 => $user->hoaDons()->where('trang_thai', 3)->count(), // Đang chuẩn bị
            4 => $user->hoaDons()->where('trang_thai', 4)->count(), // Đang giao
            5 => $user->hoaDons()->where('trang_thai', 5)->count(), // Đã giao
            6 => $user->hoaDons()->where('trang_thai', 6)->count(), // Đã hủy
            7 => $user->hoaDons()->where('trang_thai', 7)->count(), // Đã nhận hàng
        ];

        return view('clients.taikhoan.donhang', compact('danhMucs', 'donHangs', 'trang_thai_don_hang', 'counts'));
    }
    ...
    public function filterOrders(Request $request)
    {
        ...
        $donHangs = $donHangs->orderByDesc('created_at')->get();
