...
@section('content')
    <div class="card">
        <div class="card-header">
            <h5 class="card-title mb-0">Tạo tài khoản admin</h5>
        </div>
        <div class="card-body">
            <form class="row g-3" action="{{ route('admin.admins.store') }}" method="POST" enctype="multipart/form-data">
                @csrf

                <div class="col-md-6">
                    <label for="ten" class="form-label">Tên</label>
                    <input type="text" name="ten" class="form-control @error('ten') is-invalid @enderror"
                        id="ten" value="{{ old('ten') }}" required>
                    @error('ten')
                ...
                <div class="col-md-6">
                    ...
                    <input type="password" name="mat_khau_confirmation" class="form-control" id="mat_khau_confirmation"
                        required>
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="ngay_sinh" class="form-control @error('ngay_sinh') is-invalid @enderror"
                        id="ngay_sinh" value="{{ old('ngay_sinh') }}" required>
                    @error('ngay_sinh')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="file" name="anh_dai_dien"
                        class="form-control @error('anh_dai_dien') is-invalid @enderror" id="anh_dai_dien" accept="image/*">
                    @error('anh_dai_dien')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="so_nha" class="form-control @error('so_nha') is-invalid @enderror"
                        id="so_nha" value="{{ old('so_nha') }}">
                    @error('so_nha')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function capNhatDiaChi() {
                ...
                const diaChi =
                    `${soNha ? soNha + ', ' : ''}${xa.options[xa.selectedIndex].text}, ${huyen.options[huyen.selectedIndex].text}, ${tinh.options[tinh.selectedIndex].text}`;
