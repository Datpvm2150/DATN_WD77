...
@section('content')
    <div class="card">
        <div class="card-header">
            <h5 class="card-title mb-0">Sửa tài khoản admin</h5>
        </div>
        <div class="card-body">
            <form class="row g-3" action="{{ route('admin.admins.update', $admin->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')

                <div class="col-md-6">
                    <label for="ten" class="form-label">Tên</label>
                    <input type="text" name="ten" class="form-control @error('ten') is-invalid @enderror" id="ten" value="{{ old('ten', $admin->ten) }}" required>
                    @error('ten')
                ...
                <div class="col-md-6">
                    ...
                    <input type="date" name="ngay_sinh" class="form-control @error('ngay_sinh') is-invalid @enderror" id="ngay_sinh" value="{{ old('ngay_sinh', $admin->ngay_sinh) }}" required>
                    @error('ngay_sinh')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="so_dien_thoai" class="form-control @error('so_dien_thoai') is-invalid @enderror" id="so_dien_thoai" value="{{ old('so_dien_thoai', $admin->so_dien_thoai) }}">
                    @error('so_dien_thoai')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6">
                    ...
                    @if($admin->anh_dai_dien)
                        <img src="{{ asset('storage/' . $admin->anh_dai_dien) }}" alt="Ảnh đại diện" width="100" class="mt-2">
                    @endif
                </div>
                ...
                <div class="col-md-12">
                    ...
                    <input type="text" name="dia_chi" class="form-control @error('dia_chi') is-invalid @enderror" id="dia_chi" value="{{ old('dia_chi', $admin->dia_chi) }}">
                    @error('dia_chi')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
